...
{
  int               i, j, k, best_j ;
  char*            atom_used = (char*)calloc( AtomsCount, 1 ) ;
...
SYMMETRY_ELEMENT* alloc_symmetry_element()
{
...
SYMMETRY_ELEMENT* init_ultimate_plane()
{
  SYMMETRY_ELEMENT* plane = alloc_symmetry_element() ;
...
SYMMETRY_ELEMENT* init_ultimate_axis()
{
...
int read_coordinates( FILE *in )
{
  ...
  Atoms = (ATOM*)calloc( AtomsCount, sizeof( ATOM ) ) ;
  if( Atoms == NULL ){
...
{   
  int          i ;
  char*       symmetry_code = (char*)calloc( 1, 10*(PlanesCount+NormalAxesCount+2) ) ;
  char         buf[ 100 ] ;
  ...
  if( PlanesCount + NormalAxesCount == 0 )
...
void summarize_symmetry_elements()          
{
  int          i ;     
